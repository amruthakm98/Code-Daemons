...
// {cid:9, compname:"Intuit", des:"Product Based Company", url:"data:image/png;base64,"}
// ];
...
// ];
// var obj3=[{qid:12,cid:9,qname:"Binary Array sorting",ques:"Given a binary array A[] of size N. The task is to arrange array in increasing order.",level:"Easy",time_limit:30,score:10},
// {qid:13,cid:9,qname:"Top View of Binary Tree",ques:"Top view of a binary tree is the set of nodes visible when the tree is viewed from the top. Given a binary tree, print the top view of it.",level:"Hard",time_limit:60,score:30},
// {qid:14,cid:7,qname:"Spiral Matrix",ques:"Given a 2D array, print it in spiral form.",level:"Medium",time_limit:45,score:20},
// ];

// question.insertMany(obj3,function(err,res){
//      if(err) throw error;
//      console.log("Done!!!");
//  });
...
codeapp.get('/homepage/:username',function(req,res){
    ...
    company.find({},function(err,companyr){
        res.render("homepage.ejs",{user:username,companyr:companyr});  
...
codeapp.get('/questions',function(req,res){
        question.find({},function(err,questionr){
            res.render("questions.ejs",{questionr:questionr});
        })
});

codeapp.get('/questions/:companyid',function(req,res){
        var c=req.params.companyid;
        question.find({cid:c},function(err,questionr){
            console.log(c);
            res.render("questions.ejs",{questionr:questionr});
        })
});

codeapp.get('/leaderboard',function(req,res){
    user.find({},function(err,userr)
    { 
        res.render("leaderboard.ejs",{userr:userr});  
    }).sort({total_score:1});
});
...
codeapp.post('/loginsubmit',function(req,res){
    ...
    user.find({uname:uname,pwd:password},function(err,resu){
        if(err) throw err;
        if(resu.length>0){
...
codeapp.get('/editor/:qnid',function(req,res){
        var c=req.params.qnid;
        question.find({qid:c},function(err,questionr){
            console.log(c);
            res.render("questionspecific.ejs",{questionr:questionr});
        })
});
